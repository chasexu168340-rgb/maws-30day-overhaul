...
test('Day 5 combat recipe produces readable tactical feedback', async ({ page }) => {
  ...
  expect(text, 'pressure recipe should read as physical feedback, not an abstract trigger').toMatch(/抢到一拍.+脚下乱了|脚下乱了.+抢到一拍/);
  expect(text, 'combo feedback should use player-facing body language').toContain('连段反馈');
  expect(text, 'combat feedback should not leak internal plan slot debug fields').not.toMatch(/comboSlot|planSlot|PLAN触发/);
  expect(text, 'combat feedback should not expose the old combo shout label').not.toContain('COMBO！');
