...
export const SKILLS = {
  wild_swing: { name: '野路挥拳', icon: '挥', assetKey: 'skill.wild_swing', type: 'strike', dist: ['far', 'mid', 'close'], dmg: 10, post: 8, sp: 6, ap: 1, hit: 0.66, risk: 0.18, style: 'street', desc: '没练过系统拳路，但真急眼时能把人逼退。能打赢完全没练过的人，遇到正规拳距就会漏出回收问题。' },
  push_away: { name: '推搡', icon: '推', assetKey: 'skill.push_away', type: 'dirty', dist: ['mid', 'close'], dmg: 2, post: 12, sp: 6, ap: 1, hit: 0.74, risk: 0.10, style: 'street', desc: '用肩手把人顶开，抢一点喘气和距离。不是技术胜利，只是别让局面一下子贴死。' },
  ...
  advance: { name: '前压入身', icon: '进', assetKey: 'skill.advance', type: 'footwork', dist: ['far', 'mid'], dmg: 0, post: 0, sp: 5, ap: 1, hit: 1, risk: 0.12, style: 'mma', desc: '推进距离，准备近身。等于是给下一步抱摔或短打递邀请函。' },
  retreat: { name: '后撤拉开', icon: '退', assetKey: 'skill.retreat', type: 'footwork', dist: ['mid', 'close', 'ground'], dmg: 0, post: 0, sp: 5, ap: 1, hit: 1, risk: 0.08, style: 'street', desc: '脱离近身或地面压力。活着退一步，不丢人，丢的是对面的剧本。' },
  ...
  talkdown: { name: '言语降温', icon: '谈', assetKey: 'skill.talkdown', type: 'dirty', dist: ['far', 'mid', 'close'], dmg: 0, post: 0, sp: 6, ap: 1, hit: 0.58, risk: 0.05, style: 'street', desc: '降低敌意，避免硬拼。嘴不是武器，但有时能让武器先下班。' },
  ...
  mystic: { name: '混元一气掌', icon: '玄', assetKey: 'skill.mystic', type: 'mystic', dist: ['mid', 'close'], dmg: 15, post: 6, sp: 18, ap: 2, hit: 0.42, risk: 0.28, style: 'traditional', desc: '观赏性高，压力下风险也高。能赢是绝活，打空就是现场气氛组。' },
...
export const ITEMS = {
  ...
  egg: { name: '鲜鸡蛋', icon: '蛋', assetKey: 'item.egg', cat: '食材', price: 8, type: 'ingredient', desc: '便宜、靠谱，打进碗里比打在墙上有用。' },
  greens: { name: '一把青菜', icon: '菜', assetKey: 'item.greens', cat: '食材', price: 7, type: 'ingredient', desc: '不是健身神物，只是让这顿饭别只剩碳水和嘴硬。' },
  noodles: { name: '挂面', icon: '面', assetKey: 'item.noodles', cat: '食材', price: 11, type: 'ingredient', desc: '出租屋厨房的稳定输出，十分钟能把饥饿变成正经问题。' },
  home_meal: { name: '鸡蛋青菜面', icon: '碗', assetKey: 'item.home_meal', cat: '自制餐饮', price: 0, shop: false, type: 'food', desc: '热汤、鸡蛋和青菜。不是增益药水，是开打前终于吃了顿人饭。', gain: { sp: 18, fatigue: -5 }, prep: { slot: 'meal', sp: 10, morale: 2, label: '热饭打底' } },
  ice_pack: { name: '一次性冰袋', icon: '冰', assetKey: 'item.ice_pack', cat: '恢复用品', price: 28, type: 'medicine', desc: '压住训练后的肿痛，让下一场别从关节报警开始。严重伤势仍要去理疗。', gain: { hp: 8, fatigue: -6 }, prep: { slot: 'recovery', posture: 8, label: '关节冷敷' } },
  pain_gel: { name: '外用止痛凝胶', icon: '药', assetKey: 'item.pain_gel', cat: '恢复用品', price: 52, type: 'medicine', desc: '缓解轻伤，不会把伤病从存档里抹掉。疼得不对就去理疗，别拿药顶剧情。', gain: { hp: 12, injury: -1 } },
...
export const ACTIONS = {
  home: [
    ...
    { id: 'cook_egg_noodles', name: '下碗鸡蛋青菜面', icon: '灶', time: 30, sp: 2, desc: '用鸡蛋、青菜和挂面做一顿热饭。当天一次，吃下后给下一场战斗打底。', type: 'craft', noDurationOptions: true, dailyGate: 'cook_home_meal', recipe: { consumes: { egg: 1, greens: 1, noodles: 1 }, creates: { home_meal: 1 } } },
