...
const ANIM_BY_FIGHTER = {
  ...
  'fighter.enemy.challenger': 'anim.fighter.enemy.challenger',
...
export class ShellScene extends PhaserScene {
  ...
  fighterActionAnimName(step, actor) {
    ...
    if (actor?.animKey === 'anim.fighter.enemy.karate') {
      if (id === 'advance') return 'advance';
      if (id === 'karate_reverse_punch') return 'reversepunch';
      if (id === 'karate_front_kick') return 'frontkick';
      if (id === 'guard') return 'guard';
      if (['dodge', 'retreat'].includes(id)) return 'dodge';
    }
    ...
    if (actor?.animKey === 'anim.fighter.enemy.challenger') {
      if (id === 'advance') return 'advance';
      if (id === 'straight') return 'straight';
      if (id === 'mystic') return 'mystic';
      if (id === 'talkdown') return 'talkdown';
    ...
    if (actor?.animKey === 'anim.fighter.enemy.boss') {
      if (['jab', 'straight'].includes(id)) return 'boxing';
      if (id === 'frontkick') return 'frontkick';
      if (id === 'advance') return 'advance';
      if (id === 'grip') return 'clinch';
      if (id === 'takedown') return 'takedown';
      if (id === 'sprawl') return 'sprawl';
      if (id === 'guard') return 'guard';
  ...
  combatContactMs(step, fx = {}) {
    ...
    if (this.model?.combat?.enemyId === 'E11') {
      if (id === 'straight') return 320;
      if (id === 'mystic') return 360;
    }
